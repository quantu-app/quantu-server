...
RSpec.describe 'Quizzes API', type: :request do
  ...
  let(:user2) { create(:user, username: 'seconduser4') }
  let(:jwt_token) do
  ...
  describe 'list' do
    it 'returns all quizzes that belong to the user, and only those quizzes that belong to him.' do
      quizzes = create_list(:quiz, 2, user:)
      create_list(:quiz, 3, user: user2)

      get('/api/quizzes', headers:, as: :json)
      expect(json.length).to be(2)
      expect(json.pluck('name')).to match_array(quizzes.pluck('name'))
    end
  end

  describe 'create' do
    it 'creates a new quiz when given a valid name parameter' do
      post('/api/quizzes', params: { name: 'Test Quiz' }, headers:, as: :json)
  ...
  describe 'show' do
    it 'returns a quiz that belongs to the user' do
      ...
      get("/api/quizzes/#{quiz.id}", headers:, as: :json)
  ...
  describe 'update' do
    it 'updates a quiz that belongs to a user' do
      quiz = create(:quiz, name: 'a test name', user:)

      patch("/api/quizzes/#{quiz.id}", params: { name: 'another test name' }, headers:, as: :json)
  ...
  describe 'delete' do
    it 'deletes a quiz that belongs to a user' do
      ...
      delete("/api/quizzes/#{quiz.id}", headers:, as: :json)
